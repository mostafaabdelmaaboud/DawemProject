...
@Component({
  selector: 'app-penalties-default',
  templateUrl: './penalties-default.component.html',
  styleUrls: ['./penalties-default.component.scss']
})
export class PenaltiesDefaultComponent {
  date!: Date;
  arabic: any;
  ...
  columns: any[] = [
    {
      name: "كود نوع الجزئية",
      field: "code",
    },
    {
      name: "اسم نوع الجزئية",
      field: "name",
    },
    {
      name: "الحالة",
  ...
  exportTableToExcel() {
    ...
    var options = { 
      ...
      showLabels: true, 
      showTitle: true,
      title: 'انواع الجزيئات الأفتراضيه',
      useBom: true,
    ...
    if(!this.isLoading) {
      ...
      this.defaultLookupsService.getPenalties(filteration).subscribe(
        {
          next: data => {
  ...
  sendRequest(data: any) {
    ...
    this.defaultLookupsService.acceptPenalties({ PenaltiesId: data.id }).subscribe(
      {
        next: res => {
          this.getLookups(this.filteration);
  ...
  reasonOfRefuse(data: any) {
    let reasonOfRefuseDialog = this.dialog.open(DialogDeleteComponent, {
        width: "30vw",
        data: {
          title: "هل متأكد من حذف نواع الجزئية؟",
    ...
    reasonOfRefuseDialog.componentInstance.submitClicked.subscribe(result => {
      ...
      this.defaultLookupsService.deletePenalties({ PenaltiesId: data.id }).subscribe({
        next: res => {
  ...
  requestLookup() {
    ...
    dialogRefAddCurrency.componentInstance.submitClicked.subscribe(result => {
      ...
      this.defaultLookupsService.createPenalties(formData).subscribe(
        {
          next: (data: any) => {
            ...
            const succressDialog = this.dialog.open(ToastSuccessComponent, {
              ...
              data: {
                ...
                buttonSend: "انواع الجزيئات الأفتراضية"
  ...
  editLookup(data: any) {
    const dialogRefAddCurrency = this.dialog.open(AddLookupComponent, {
      ...
      data: {
        title: "تعديل نوع الجزئية ",
        label:"إسم نوع الجزئية",
        setAsNecessary: "تعيين كنشط",
    ...
    dialogRefAddCurrency.componentInstance.typeGetById = 8;
    ...
    dialogRefAddCurrency.componentInstance.submitClicked.subscribe(result => {
      ...
      formData.isActive = result.IsActive;
      dialogRefAddCurrency.componentInstance.submitted = false;
      this.defaultLookupsService.updatePenalties(formData).subscribe(
        {
          next: (data: any) => {
            ...
            const succressDialog = this.dialog.open(ToastSuccessComponent, {
              ...
              data: {
                title: "تم ارسال طلبك",
                message: data.message,
                buttonSend: "نوع الجزئية الأفتراضية"
  ...
  dialogLookupFile(data: any) {
    const dialogRefAddCurrency = this.dialog.open(LookupFileComponent, {
      width: "40vw",
      data: {
        title: "ملف نوع الجزئية",
        codeLabel:"كود نوع الجزئية ",
        translationsLabel:"أسماء نوع الجزئية",
        nameLabel:"أسم نوع الجزئية "
      },
    });
    dialogRefAddCurrency.componentInstance.id = data.id;
    dialogRefAddCurrency.componentInstance.infoType =8;
