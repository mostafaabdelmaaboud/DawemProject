...
@Component({
  selector: 'app-shift-type-default',
  templateUrl: './shift-type-default.component.html',
  styleUrls: ['./shift-type-default.component.scss']
})
export class ShiftTypeDefaultComponent {
  date!: Date;
  arabic: any;
  ...
  columns: any[] = [
    {
      name: "كود نوع الوردية",
      field: "code",
    },
    {
      name: "اسم نوع الوردية",
      field: "name",
    },
    {
      name: "الحالة",
  ...
  exportTableToExcel() {
    ...
    var options = { 
      ...
      showLabels: true, 
      showTitle: true,
      title: 'انواع الورديات الأفتراضيه',
      useBom: true,
    ...
    if(!this.isLoading) {
      ...
      this.defaultLookupsService.getShiftType(filteration).subscribe(
        {
          next: data => {
  ...
  sendRequest(data: any) {
    ...
    this.defaultLookupsService.acceptShiftType({ ShiftTypeId: data.id }).subscribe(
      {
        next: res => {
          this.getLookups(this.filteration);
  ...
  reasonOfRefuse(data: any) {
    let reasonOfRefuseDialog = this.dialog.open(DialogDeleteComponent, {
        width: "30vw",
        data: {
          title: "هل متأكد من حذف نواع الورديات؟",
    ...
    reasonOfRefuseDialog.componentInstance.submitClicked.subscribe(result => {
      ...
      this.defaultLookupsService.deleteShiftType({ ShiftTypeId: data.id }).subscribe({
        next: res => {
  ...
  requestLookup() {
    ...
    dialogRefAddCurrency.componentInstance.submitClicked.subscribe(result => {
      ...
      this.defaultLookupsService.createShiftType(formData).subscribe(
        {
          next: (data: any) => {
            ...
            const succressDialog = this.dialog.open(ToastSuccessComponent, {
              ...
              data: {
                ...
                buttonSend: "نوع الورديات الأفتراضية"
  ...
  editLookup(data: any) {
    const dialogRefAddCurrency = this.dialog.open(AddLookupComponent, {
      ...
      data: {
        title: "تعديل نوع الوردية ",
        label:"إسم نوع الوردية",
        setAsNecessary: "تعيين كنشط",
    ...
    dialogRefAddCurrency.componentInstance.typeGetById = 7;
    ...
    dialogRefAddCurrency.componentInstance.submitClicked.subscribe(result => {
      ...
      formData.isActive = result.IsActive;
      dialogRefAddCurrency.componentInstance.submitted = false;
      this.defaultLookupsService.updateShiftType(formData).subscribe(
        {
          next: (data: any) => {
            ...
            const succressDialog = this.dialog.open(ToastSuccessComponent, {
              ...
              data: {
                title: "تم ارسال طلبك",
                message: data.message,
                buttonSend: "نوع الوردية الأفتراضية"
  ...
  dialogLookupFile(data: any) {
    const dialogRefAddCurrency = this.dialog.open(LookupFileComponent, {
      width: "40vw",
      data: {
        title: "ملف نوع الوردية",
        codeLabel:"كود نوع الوردية ",
        translationsLabel:"أسماء نوع الوردية",
        nameLabel:"أسم نوع الوردية "
      },
    });
    dialogRefAddCurrency.componentInstance.id = data.id;
    dialogRefAddCurrency.componentInstance.infoType =7;
