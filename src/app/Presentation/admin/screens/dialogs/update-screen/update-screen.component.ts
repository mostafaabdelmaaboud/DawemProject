...
export class UpdateScreenComponent {
  ...
  constructor(
    public dialogRef: MatDialogRef<UpdateScreenComponent>,
  ...
  getLanguages() {
    ...
    this.screensService.getLanguages(this.filterationLanguages).subscribe({
      next:res => {
  ...
  request() {
    ...
    if (this.addBranchGroupForm.valid && this.submitted && this.getControlArray("NameTranslations").at(this.getControlArray("NameTranslations").controls.length - 1).valid) {
      ...
      this.getControl("URL")?.markAsDirty();
    ...
    switch (type) {
        ...
        if (data.value || data.value === "") {
          if (data.value !== this.lastSearchQuery || data.value === "") {
            ...
            this.screensService.getLanguages({ PagingEnabled: true, PageSize: 5, PageNumber: 0, FreeText: data.value }).pipe(
              debounceTime(300),
