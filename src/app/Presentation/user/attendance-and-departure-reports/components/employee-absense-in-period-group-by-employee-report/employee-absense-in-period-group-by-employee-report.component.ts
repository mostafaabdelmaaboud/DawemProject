...
@Component({
  selector: 'app-employee-absense-in-period-group-by-employee-report',
  templateUrl: './employee-absense-in-period-group-by-employee-report.component.html',
  styleUrls: ['./employee-absense-in-period-group-by-employee-report.component.scss']
})
export class EmployeeAbsenseInPeriodGroupByEmployeeReportComponent {
  date!: Date;
  ...
  getReport(filteration) {
    ...
    this.reportService.getEmployeeAbsenseInPeriodGroupByEmployeeReport(filteration).subscribe({
      next: (blob: Blob) => {
