...
import { RecallReportsService } from './services/recall-reports.service';

@Component({
  selector: 'app-recall-reports',
  templateUrl: './recall-reports.component.html',
  styleUrls: ['./recall-reports.component.scss']
})
export class RecallReportsComponent {
  date!: Date;
  ...
  private recallReportsService = inject(RecallReportsService);
  show = false;
  ...
  getReport(filteration) {
    ...
    this.recallReportsService.GetEmployeeDailyAttendanceGroupByDayPath(filteration).subscribe({
      next: (blob: Blob) => {
