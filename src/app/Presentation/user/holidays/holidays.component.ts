  ...
  exportTableToExcel() {
    ...
    if(!this.isLoading) {
      ...
      this.holidaysService.listHolidays(filteration).subscribe(data => {
        ...
        this.isLoading = false;
        let formatRows =formatTable.map(holiday => [holiday.code,holiday.name, holiday.dateType, holiday.startDate, holiday.endDate ]);
        this.generateExcel('العطلات الرسمية','العطلات الرسمية',formatRows, columns);
