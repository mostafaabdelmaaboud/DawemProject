...
import { DelayReportsService } from './services/delay-reports.service';

@Component({
  selector: 'app-delay-reports',
  templateUrl: './delay-reports.component.html',
  styleUrls: ['./delay-reports.component.scss']
})
export class DelayReportsComponent {
  date!: Date;
  ...
  private delayReportsService = inject(DelayReportsService);
  show = false;
  ...
  getReport(filteration) {
    ...
    this.delayReportsService.GetEmployeeDailyAttendanceGroupByDayPath(filteration).subscribe({
      next: (blob: Blob) => {
