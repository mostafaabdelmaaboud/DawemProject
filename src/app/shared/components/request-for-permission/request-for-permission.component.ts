...
@Component({
  selector: 'app-request-for-permission',
  standalone: true,
  ...
  templateUrl: './request-for-permission.component.html',
  styleUrls: ['./request-for-permission.component.scss']
})
export class RequestForPermissionComponent {
  ...
  ngOnInit(): void {
    ...
    combineLatest({
      ...
      if (this.editPermission) {
        ...
        this.permissionsService.PermissionGetById({ requestId: this.id }).subscribe(
          {
            next: data => {
              ...
              this.permissionsService.permissionTypeDropdown({ PagingEnabled: true, PageSize: 5, PageNumber: 0, id: data?.permissionTypeId }).subscribe(dataDropdown => {
      ...
      case 'PermissionTypeId':
        if (data || data === "") {
          if (data !== this.lastSearchQuery || data === "") {
            ...
            this.permissionsService.permissionTypeDropdown({ PagingEnabled: true, PageSize: 5, PageNumber: 0, FreeText: data }).pipe(
              debounceTime(300),
