...
@Component({
  selector: 'app-dialog-schedule-plan-file',
  standalone: true,
  ...
  templateUrl: './dialog-schedule-plan-file.component.html',
  styleUrls: ['./dialog-schedule-plan-file.component.scss']
})
export class DialogSchedulePlanFileComponent {
  loading = false;
  ...
  constructor(
    public dialogRef: MatDialogRef<DialogSchedulePlanFileComponent>,
  ...
  ngOnInit(): void {
    ...
    if (this.id) {
      ...
      this.schedualPlanService.schedualPlanGetInfo({ schedulePlanId: this.id }).subscribe(data => {
        this.info = data;
