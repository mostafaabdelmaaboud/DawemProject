...
@Component({
  selector: 'app-dialog-assignment-type-file',
  standalone: true,
  ...
  templateUrl: './dialog-assignment-type-file.component.html',
  styleUrls: ['./dialog-assignment-type-file.component.scss']
})
export class DialogAssignmentTypeFileComponent {
  loading = false;
  ...
  constructor(
    public dialogRef: MatDialogRef<DialogAssignmentTypeFileComponent>,
  ...
  ngOnInit(): void {
    ...
    if (this.id) {
      this.assignmentTypeService.assignmentGetInfo({ assignmentTypeId: this.id }).subscribe(
        {
          next: data => {
            this.info = data;
            this.loading = false;
