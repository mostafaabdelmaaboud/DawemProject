...
@Component({
  selector: 'app-request-for-overtime',
  standalone: true,
  ...
  templateUrl: './request-for-overtime.component.html',
  styleUrls: ['./request-for-overtime.component.scss']
})
export class RequestForOvertimeComponent {
  ...
  constructor(
    public dialogRef: MatDialogRef<RequestForOvertimeComponent>,
  ...
  ngOnInit(): void {
    ...
    let vacationTypeForDropDown = this.overtimeRequestService.overtimeTypeDropdown({ PagingEnabled: true, PageSize: 5, PageNumber: 0 });
    ...
    combineLatest({
      ...
      if (this.editPermission) {
        ...
        this.overtimeRequestService.overtimeGetById({ requestId: this.id }).subscribe(
          {
            next: data => {
              ...
              this.overtimeRequestService.overtimeTypeDropdown({ PagingEnabled: true, PageSize: 5, PageNumber: 0, id: data?.permissionTypeId }).subscribe(dataDropdown => {
                ...
                let indexpermissionTypeId = this.list.findIndex(job => job.key === data.permissionTypeId);
                if (indexpermissionTypeId >= 0) {
                  this.addBranchGroupForm.get("PermissionTypeId")?.setValue(this.list[indexpermissionTypeId]);
                }
              });
      ...
      case 'PermissionTypeId':
        if (data || data === "") {
          if (data !== this.lastSearchQuery || data === "") {
            ...
            this.overtimeRequestService.overtimeTypeDropdown({ PagingEnabled: true, PageSize: 5, PageNumber: 0, FreeText: data }).pipe(
              debounceTime(300),
